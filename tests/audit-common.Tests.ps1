...
Describe 'Foundation Library Loading' {
    ...
    It 'COMMON_LIB_VERSION matches the orchestrator ScriptVersion' {
        $script:COMMON_LIB_VERSION | Should -Be $script:ExpectedVersion
    }
...
Describe 'Get-AuditCommonInfo' {
    ...
    It 'Includes Version field matching the orchestrator ScriptVersion' {
        $info = Get-AuditCommonInfo
        $info.Version | Should -Be $script:ExpectedVersion
    }
